...
$okIndeterminate = ($out -match 'handing off to agent process') -and ($out -notmatch '\d+%')
Write-Host ("cat-art={0} face={1} label={2} agent-ran={3} no-fake-percent={4}" -f $okCat, $okFace, $okLabel, $okAgent, $okIndeterminate)
if ($okCat -and $okFace -and $okLabel -and $okAgent -and $okIndeterminate) { Write-Host 'ALL PASS' } else { Write-Host 'FAIL'; Write-Host $out; exit 1 }
